...
export(Resource) var wild_pokemon
...
onready var wild_pokemon_graphics_ = find_node("enemy")
...
func _ready():
	...
	wild_pokemon_graphics_.pokemon.replace_by_instance(wild_pokemon.battle_graphics)
...
func get_next_enemy_move_() -> int:
	return int(round(rand_range(0, wild_pokemon.moves.size() - 1)))
...
func apply_swap_pokemon_(trainer:NPCModel, pokemon_idx:int, graphics:Node) -> void:
...
func apply_player_attack_(move_idx:int) -> void:
	apply_attack_(player.active_pokemon, wild_pokemon, get_node("player"), get_node("enemy"), move_idx)

func apply_enemy_attack_(move_idx:int) -> void:
	apply_attack_(wild_pokemon, player.active_pokemon, get_node("enemy"), get_node("player"), move_idx)
...
func apply_attack_(attacking_pokemon:PokemonModel, defending_pokemon:PokemonModel, attacking_graphics:Node, defending_graphics:Node, move_idx:int) -> void:
	...
	var e := "Enemy " if attacking_pokemon == wild_pokemon else ""

	yield(info_box_.set_text("%s%s used %s!" % [e, attacking_pokemon.name, move.name]), "done")
	if move.fx:
		var fx = move.fx.instance()
		fx.defender_graphics = defending_graphics.find_node("pokemon_position")
		fx.attacker_graphics = attacking_graphics.find_node("pokemon_position")
...
func game() -> void:
	...
	wild_pokemon_graphics_.pokemon.show_front()
	player_graphics_.trainer.begin($tween)
	wild_pokemon_graphics_.pokemon.begin($tween)
	yield($tween.block(), "done")
	
	find_node("stats").set_from_pokemon(wild_pokemon)
	find_node("stats").visible = true

	yield(info_box_.set_text_for_confirm("wild %s appeared" % wild_pokemon.name), "done")
	info_box_.clear_text()
	...
	while true:
		...
		match player_action.type:
			...
			Action.Type.swap:
				...
				yield(self, "action_applied")
			Action.Type.run:
				yield(info_box_.set_text_for_confirm("Got away safely"), "done")
				break
				

		if wild_pokemon.is_dead():
			wild_pokemon_graphics_.stats.visible = false
			yield(wild_pokemon_graphics_.get_pokemon().faint(), "done")
			yield(info_box_.set_text_for_confirm("Enemy %s fainted!"), "done")

			var last_level = player.active_pokemon.level
			player.active_pokemon.xp += wild_pokemon.get_exp_if_beat()
			yield(info_box_.set_text_for_confirm("%s gained %d EXP." % [player.active_pokemon.name, wild_pokemon.get_exp_if_beat()]), "done")
			...
			if wild_pokemon.is_dead():
				yield(info_box_.set_text_for_confirm("%s is defeated!"), "done")
				break
